...
global.Firemd = 'https://shizoapi.onrender.com'
...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('initializing FIRE MD Bot')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    const msg = `DEAR FIRENITE🔥 ${name},FIRE MD 🔥 CONNECTED SUCCESSFULLY © opaksfire 😊 FIRE MD IS HERE TO OVERTAKE🔥 ✅\nFOLLOW OUR OFFICIAL CHANNEL \n https://whatsapp.com/channel/0029VajYjMJJf05aRQXKx82W`

    await conn.sendMessage(jid, { text: msg, mentions: [jid] }, { quoted: null })

    conn.logger.info(chalk.yellow('\nFIRE MD 🔥 CONNECTED SUCCESSFULLY © opaksfire 😊 ✅'))
  }
